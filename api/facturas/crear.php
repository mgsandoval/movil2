<?php
// api/facturas/crear.php
// Inserta una factura (encabezado + detalles) dentro de una transacción.
...
// Campos del encabezado (todos con valor por defecto: el OCR puede traer datos parciales)
...
// Normalizar la fecha (acepta 'dd/mm/yyyy' o 'yyyy-mm-dd [hh:ii:ss]')
...
try {
    $sqlEnc = "INSERT INTO tbl_factura_encabezado
        (factura_numero, factura_cai, factura_rtn_local, factura_nombre_local, factura_direccion,
         ...
         usuario_id, factura_categoria)
        VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
    $stmt = $conexion->prepare($sqlEnc);
    if (!$stmt) throw new Exception('prepare encabezado: ' . $conexion->error);

    // 22 params: 7 strings, 10 decimals, totalArt(i), pago(d), cambio(d), usuarioId(i), categoria(s)
    $stmt->bind_param(
        'sssssssddddddddddiddis',
    ...
    if (!$stmt->execute()) throw new Exception('insert encabezado: ' . $stmt->error);
    ...
    if (!empty($detalles)) {
        ...
        if (!$stmtDet) throw new Exception('prepare detalle: ' . $conexion->error);
    ...
    echo json_encode(['status' => 'error', 'message' => 'No se pudo guardar la factura', 'error' => $e->getMessage()], JSON_UNESCAPED_UNICODE);
}
$conexion->close();
?>
